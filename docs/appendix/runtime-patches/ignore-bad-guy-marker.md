...
At static address `0x00431F85` (RVA `0x00031F85`), replace:

```diff
- 000: C6 82 68 06 00 00 01 | mov byte ptr [edx+0x668], 1 ; apply the local Bad Guy marker
+ 000: C6 82 68 06 00 00 00 | mov byte ptr [edx+0x668], 0 ; keep normal endpoint and login behavior
```
