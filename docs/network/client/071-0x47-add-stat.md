# 071 / 0x47: CAddStatPacket?

- Direction: client to server
- Internal packet name: `CAddStatPacket?`
- Local behavioral alias: `AddStat / RaiseStat`
- Related-game enum name: `AddStat`
- Name provenance: Related-game internal enum name `AddStat` at the same opcode matches this client's stat-selector payload better than the later leaked `CGrowSkillPacket` remap; the C++ class spelling remains reconstructed.
...
- The same-opcode internal enum name `AddStat` therefore fits better than the later-game `CGrowSkillPacket` class remap.
