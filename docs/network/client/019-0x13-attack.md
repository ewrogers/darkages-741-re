...
`ui_world_pane_pursue_and_auto_attack_target` resolves a selected object ID, paths to an adjacent tile, and repeats its face-or-attack check every 100 ms. Once adjacent and already facing the target, it calls the same opcode-only builder without the Space-key throttle. See [Pathfinding and following](../../systems/pathfinding-and-pursuit.md).
