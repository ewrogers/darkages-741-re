...
The [Good Guy runtime patch](../../appendix/runtime-patches.md#ignore-the-local-bad-guy-marker-good-guy) changes the marker-present branch to write `0` to the flag. A client left unusable by the marker can then use the normal port and submit `CLogin` again without deleting the file.
