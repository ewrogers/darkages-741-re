...
`net_dispatch_received_message` retains both the raw post-decrypt body and, when available, the packet object created by `net_deserialize_server_packet`. `net_dispatch_game_server_message` at `0x5ED990` uses both representations. Consequently, an opcode may be supported even when no factory is registered for it.
