...
The compiled `MerchantDialogPane::ServerItemMenuDialog3` family contains the same name-based `DescPane` pattern, including a row-relative timer callback. Its construction branch is present in `ui_open_server_item_menu_dialog`, and the class parses items, groups them into metadata categories, pages four rows at a time, and loads `lmerc2.txt`. The build selector `ui_is_server_item_menu_dialog3_enabled` always returns false, so this client selects the older merchant dialog instead. The newer family is complete but dormant in the matching build.

The current local-inventory screen-menu types 5 and 11 use a different list family and have no direct `DescPane` call in the checked path. The exact mapping of buy, sell, deposit, and withdrawal screens to the active older families therefore remains partly behavioral evidence.
