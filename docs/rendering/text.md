...
Exact RTTI `InputMan` owns a hidden default IME window and a context associated with the main client window. Its composition reader reserves two bytes, calls the ANSI API, releases the temporary context, and writes two zero terminators. Focus changes notify the old and new live panes rather than transferring raw IME pointers between controls.
