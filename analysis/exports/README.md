...
[`startup.yaml`](startup.yaml) establishes schema version `1`. [`network.yaml`](network.yaml) records connection analysis, [`transport.yaml`](transport.yaml) records the shared send, receive, framing, and transform pipeline, and [`packets.yaml`](packets.yaml) adds deterministic direction-specific opcode indexes. All retain the same required manifest fields. Each manifest records the exact target fingerprint and focused evidence. Lists are sorted by address or opcode. Addresses and byte strings use uppercase hexadecimal text so reviews do not depend on YAML number handling.
